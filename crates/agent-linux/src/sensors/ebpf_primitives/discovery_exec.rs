// linux/sensors/ebpf_primitives/discovery_exec.rs
// Detects discovery/reconnaissance tool execution (whoami, id, ps, netstat, etc.)
...
/// Detect discovery tool execution from exec events
/// Triggers on: whoami, id, uname, ps, lsof, netstat, ss, ip, ifconfig, hostname, groups, getent, cat
pub fn detect_discovery_exec(base_event: &Event) -> Option<Event> {
    let discovery_tools = [
        "whoami", "id", "uname", "ps", "lsof", "netstat", "ss", "ip", "ifconfig", "hostname",
        "groups", "getent", "cat",
    ];
    ...
    // Check if exe matches any discovery tool
