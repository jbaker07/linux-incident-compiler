...
pub enum FactType {
    ...
    /// File deletion
    DeletePath { proc_key: String, path: String },
...
impl Fact {
    ...
    pub fn compute_fact_id(
        ...
        let type_str = match fact_type {
            ...
            FactType::WritePath { proc_key, path, .. } => format!("write:{}:{}", proc_key, path),
            FactType::ReadPath { proc_key, path, .. } => format!("read:{}:{}", proc_key, path),
            FactType::CreatePath { proc_key, path, .. } => format!("create:{}:{}", proc_key, path),
            FactType::DeletePath { proc_key, path, .. } => format!("delete:{}:{}", proc_key, path),
            FactType::PersistArtifact { proc_key, path, .. } => {
                format!("persist:{}:{}", proc_key, path)
            }
            FactType::PrivilegeBoundary {
                proc_key,
                uid_before,
                uid_after,
                ..
            } => format!("priv:{}:{}:{}", proc_key, uid_before, uid_after),
            FactType::MemWX { proc_key, addr, .. } => format!("memwx:{}:{:x}", proc_key, addr),
            FactType::ModuleLoad {
                proc_key,
                module_path,
                ..
            ...
            FactType::KernelModule { module_name, .. } => format!("kmod:{}", module_name),
            FactType::LogCleared { log_name, .. } => format!("logclear:{}", log_name),
            FactType::AuthEvent {
                user_key,
                auth_type,
                ..
    ...
    pub fn domain(&self) -> FactDomain {
        match &self.fact_type {
            ...
            | FactType::ReadPath { .. }
            | FactType::CreatePath { .. }
            | FactType::DeletePath { .. } => FactDomain::File,
            FactType::PersistArtifact { .. } => FactDomain::Persist,
            FactType::PrivilegeBoundary { .. } | FactType::AuthEvent { .. } => FactDomain::Auth,
            FactType::MemWX { .. }
            | FactType::RemoteThread { .. }
            | FactType::DebugAttach { .. } => FactDomain::Memory,
            FactType::KernelModule { .. } | FactType::LogCleared { .. } => FactDomain::Tamper,
        }
    }

    /// Check if this fact has visibility gaps
    pub fn has_visibility_gaps(&self) -> bool {
        !self.visibility_gaps.is_empty() || self.fields.values().any(|v| v.is_unknown())
    }
...
impl FactBuilder {
    ...
    pub fn build(self) -> Fact {
        let fact_id =
            Fact::compute_fact_id(&self.host_id, &self.scope_key, &self.fact_type, self.ts);
